...
# 读取json文件内容,返回字典格式
with open('result_residual_6_9.json', 'r', encoding='utf8')as fp:
    result = json.load(fp)
...
for T in T_list:
    ...
    while result_index < len(result):
        ...
        while turn_index <= result[result_index]['turn_num']:
            ...
            # 生成words
            bspn_words = constraint_dict_to_list(
                bspan_to_constraint_dict(result[result_index + turn_index]['bspn']))
            ...
            # print("bspn_words_gen:", bspn_words_gen)
            bspn_prob_gen = get_words_prob(bspn_words_gen, bspn_tokens_gen, bspn_prob_gen)
            ...
            # 得到confidence所在区间
            distance, bspn_right_array = levenshtein(bspn_words, bspn_words_gen)
...
data.to_excel('unsorted_ECE' + str(T_begin) + '_' + str(T_end) + '.xlsx')
##
# 画图
...
data.to_excel('unsorted_data.xlsx')
...
plot_acc_con_figure(confidence_array, accuracy_array, "unsorted")
##
# # 第一次应将键值修整的整洁
# if T == 1.0:
