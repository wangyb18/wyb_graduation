...
class Modal(object):
    def __init__(self, device=[0]):
        ...
        if cfg.mode in single_mode:
            ...
            self.noise_model = GPT2LMHeadModel.from_pretrained(cfg.noise_path)
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    try:  # avoid OOM
                        ...
                        if cfg.only_target_loss:
                            ...
                            loss, energy_pos = self.calculate_loss_and_accuracy(
                                outputs_pos, labels=labels_pos['contexts_tensor'])
                            temp_loss, energy_neg = self.calculate_loss_and_accuracy(
                        ...
                        print("energy_neg:", energy_neg)

                        if nce:
                            p_pos = 1/(1+K*torch.exp(energy_pos))
                            p_neg = K/(K+torch.exp(-energy_neg))
                            nce_N0 = -torch.log(p_pos)
                            nce_N1 = -K*torch.log(p_neg)
    ...
    def calculate_loss_and_accuracy(self, outputs, labels, type='normal'):
        ...
        energy = -energy_logits.sum(dim=1).squeeze()/unpad_labels.sum(dim=1)
        print("2 energy_logits:", energy)
