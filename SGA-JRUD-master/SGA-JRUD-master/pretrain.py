                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    try:  # avoid OOM
                        ...
                        if cfg.only_target_loss:
                            ...
                            loss = self.calculate_loss_and_accuracy(
                                outputs, labels=labels['contexts_tensor'])
                        else:
                            loss = self.calculate_loss_and_accuracy(
...
def main():
    if not os.path.exists('./pre_experiments'):
        os.mkdir('./pre_experiments')

    if not os.path.exists('./pre_experiments_21'):
        os.mkdir('./pre_experiments_21')
    ...
    if 'test' in args.mode:
        ...
        if cfg.exp_path in ['', 'to be generated']:
            experiments_path = './pre_experiments_21' if cfg.dataset == 1 else './pre_experiments'
            if cfg.exp_no == '':
